  ...
  try {
    ...
    app.get("/users", async (req, res) => {
      try {
        ...
        // If email query exists, find specific user
        if (email) {
          const query = { email: email };
          ...
          res.send(user); // Send single user object
        } else {
          // If no email, return all users
          const cursor = userCollection.find();
          const result = await cursor.toArray();
          res.send(result); // Send array of users
        }
      } catch (error) {
        res.status(500).send({ error: error.message });
      }
    ...
    app.patch("/users/:id", async (req, res) => {
      try {
        ...
        const result = await userCollection.updateOne(query, updatedProfile);
    ...
    console.log(
      "Pinged your deployment. You successfully. You successfully connected to MongoDB!"
    );
  } finally {
  }
}
